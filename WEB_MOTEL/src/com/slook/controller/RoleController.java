...
public class RoleController
{
    ...
    public TreeNode initCheckboxFunctionPath(List<FunctionPath> lstFunctionPaths)
    {
        ...
        {
            lstTotal.forEach((bo) -> {
                if (!Constant.FUNCTION_PATH.TYPE_ACTION.equals(bo.getType()))
                {// bo qua cac node action
    ...
    public void createTreeFunctionPath(TreeNode parentNode, List<FunctionPath> lstTotal)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            {
                if (!Constant.FUNCTION_PATH.TYPE_ACTION.equals(lstTotal.get(i).getType()))
                {// bo qua cac node action
    ...
    public Long createTreeAction(TreeNode parentNode, List<FunctionPath> lstTotal, List<Long> lstFunctionPathIdSelect)
    {
        ...
        for (int i = 0; i < size; i++)
        {
                    ...
                    && lstTotal.get(i).getParentId().equals(((FunctionPath) parentNode.getData()).getFunctionPathId())))
            {
                ...
                if (Constant.FUNCTION_PATH.TYPE_FUNCTION.equals(lstTotal.get(i).getType())
                        && !lstFunctionPathIdSelect.contains(lstTotal.get(i).getFunctionPathId()))
                {// chuc nang khong duoc gan quyen thi bo di
                    continue;
                }
                lstNode.add(node);
                lstTotal.remove(i);
        ...
        {
            if (!Constant.FUNCTION_PATH.TYPE_ACTION.equals(((FunctionPath) parentNode.getData()).getType()))
            {// chi them voi node hien tai la chuc nang
                Long type = createTreeAction(node, lstTotal, lstFunctionPathIdSelect);
                if (!Constant.FUNCTION_PATH.TYPE_ACTION.equals(type))
                {// xoa node  neu khong co action
    ...
    public List<TreeNode> getListTreeNodeSelect(TreeNode node, List<Long> lstFunctionPathIdSelect)
    {
        ...
        else if (lstFunctionPathIdSelect == null || lstFunctionPathIdSelect.size() == 0)
        {// unable selected
